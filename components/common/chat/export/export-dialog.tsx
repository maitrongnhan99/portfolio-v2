...
import { ConversationExporter, type ExportFormat, type ExportOptions } from "@/lib/export-utils";
...
export function ExportDialog({ 
  ...
  const setIsOpen = isControlled ? (open: boolean) => { if (!open) onClose(); } : setInternalOpen;
    ...
    try {
      ...
      if (conversation) {
        ...
        if (selectedConversations.length > 0) {
          conversationsToExport = conversations.filter(c => selectedConversations.includes(c.id));
      ...
      if (conversationsToExport.length === 1) {
        ...
        const exportData = {
          ...
          totalMessages: conversationsToExport.reduce((sum, conv) => sum + (conv.messages?.length || 0), 0)
        };
        
        const content = JSON.stringify(exportData, null, 2);
        const filename = `conversations-export-${new Date().toISOString().split('T')[0]}.json`;
        ConversationExporter.downloadFile(content, filename, 'application/json');
      }
        ...
        <DialogContent className="sm:max-w-[500px] bg-navy-light border-navy-lighter">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <Label className="text-slate-lighter font-medium">Export Format</Label>
              <Select value={selectedFormat} onValueChange={(value) => setSelectedFormat(value as ExportFormat)}>
                ...
                <SelectContent className="bg-navy-light border-navy-lighter">
                  {formatOptions.map((format) => (
                    <SelectItem key={format.value} value={format.value} className="text-slate-lighter hover:bg-navy">
                      <div className="flex items-center gap-2">
                        <format.icon className="w-4 h-4" />
                        <div>
                          <div className="font-medium">{format.label}</div>
                          <div className="text-xs text-slate/60">{format.description}</div>
                        </div>
                      </div>
                    </SelectItem>
                  ))}
            ...
            {/* Export Options */}
            <div className="space-y-3">
              <Label className="text-slate-lighter font-medium">Export Options</Label>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="metadata"
                    checked={exportOptions.includeMetadata}
                    onCheckedChange={(checked) => updateExportOption('includeMetadata', checked as boolean)}
                    className="border-navy-lighter"
                  />
                  <Label htmlFor="metadata" className="text-sm text-slate-lighter">Include metadata</Label>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => updateExportOption('includeSources', checked as boolean)}
                    className="border-navy-lighter"
                  />
                  <Label htmlFor="sources" className="text-sm text-slate-lighter">Include sources</Label>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => updateExportOption('includeTimestamps', checked as boolean)}
                    className="border-navy-lighter"
                  />
                  <Label htmlFor="timestamps" className="text-sm text-slate-lighter">Include timestamps</Label>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => updateExportOption('compressOutput', checked as boolean)}
                    className="border-navy-lighter"
                  />
                  <Label htmlFor="compress" className="text-sm text-slate-lighter">Compress output</Label>
                </div>
              </div>
            </div>
            ...
            <div className="bg-navy/50 rounded-lg p-4 border border-navy-lighter">
              <div className="flex items-center gap-2 mb-2">
                {selectedFormatInfo && <selectedFormatInfo.icon className="w-4 h-4 text-primary" />}
                <span className="text-sm font-medium text-slate-lighter">Export Preview</span>
              </div>
              <div className="space-y-1 text-xs text-slate/70">
                {conversation ? (
                  <>
                    <div>Conversation: {conversation.title}</div>
                    <div>Messages: {conversation.messageCount || conversation.messages?.length || 0}</div>
                  </>
                ) : conversations ? (
                  <>
                    <div>Conversations: {conversations.length}</div>
                    <div>Current: {currentConversation?.title || 'All conversations'}</div>
                  </>
                ) : (
                  <div>No conversation data available</div>
                )}
                <div>Format: {selectedFormatInfo?.label}</div>
                <div>Options: {Object.entries(exportOptions).filter(([_, value]) => value).map(([key]) => key).join(', ')}</div>
              </div>
            </div>
