...
import { useSidebarSearch, type SearchResult } from "@/hooks/use-sidebar-search";
...
export const SidebarSearch: React.FC<SidebarSearchProps> = ({
  ...
  }, [searchResults, selectedIndex, handleKeyNavigation, onLoadConversation, onClose, onBack]);
...
const SearchResults: React.FC<{
...
}> = ({ results, query, currentConversation, selectedIndex, onSelect, onDelete }) => {
  if (!query) {
    return (
  ...
  if (results.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-slate/60">No results found for &ldquo;{query}&rdquo;</p>
        <p className="text-xs text-slate/40 mt-2">
  ...
  const highlightText = (text: string) => {
    ...
    return parts.map((part, i) => 
      part.toLowerCase() === query.toLowerCase() ? 
        <mark key={i} className="bg-yellow-500/30 text-slate-light rounded px-0.5">{part}</mark> : 
        part
    );
  };
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {result.matchType === 'topic' && result.conversation.topicsExplored && (
            <div className="flex gap-1 mt-1 flex-wrap">
              {result.conversation.topicsExplored.slice(0, 3).map((topic, idx) => (
                <span key={idx} className="text-xs px-1.5 py-0.5 bg-navy-lighter rounded">
                  {highlightText(topic)}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 text-xs text-slate/50 mt-1">
            ...
            {result.matchType === 'message' && result.matchingMessages.length > 1 && (
              <>
                <span>•</span>
