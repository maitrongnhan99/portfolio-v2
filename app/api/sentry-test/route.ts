  ...
  try {
    switch (testType) {
      case "server":
        ...
        throw new Error(
          "Test Sentry Server Error - This is a test error for Sentry integration"
        );
      ...
      case "async":
        ...
        await new Promise((resolve, reject) => {
          setTimeout(() => {
            reject(
              new Error(
                "Test Sentry Async Error - This async error should be caught by Sentry"
              )
            );
          }, 100);
        });
      ...
      default:
        return NextResponse.json(
          {
            message:
              "Invalid test type. Use ?type=server, ?type=async, ?type=reference, or ?type=custom",
          },
          { status: 400 }
        );
    }
...
export async function POST() {
  ...
  throw new Error(
    "Test Sentry Edge Runtime Error - This is a test error for Edge runtime"
  );
}
