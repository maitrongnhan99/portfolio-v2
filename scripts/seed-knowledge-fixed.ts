...
console.log(
  "  MongoDB:",
  process.env.MONGODB_CONNECTION_STRING ? "✅ Found" : "❌ Missing"
);
console.log(
  "  Gemini:",
...
class KnowledgeSeeder {
  ...
  async seed(options: SeedOptions = {}) {
    ...
    try {
      ...
      if (clearExisting) {
        ...
        console.log(
          `✅ Deleted ${deleteResult.deletedCount} existing chunks\n`
        );
      } else {
        const existingCount = await KnowledgeChunk.countDocuments();
        if (existingCount > 0) {
          console.log(
            `ℹ️  Found ${existingCount} existing chunks (use --clear to remove)\n`
          );
        }
      }
      ...
      for (let i = 0; i < knowledgeBase.length; i += batchSize) {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          try {
            console.log(
              `  ${globalIndex}. [${chunk.category}] ${chunk.content.substring(
                0,
                60
              )}...`
            );
            ...
            successCount++;
            console.log(
              `     ✅ Embedded and saved (${embedding.length} dimensions)`
            );
          } catch (error) {
            errorCount++;
            console.error(
      ...
      console.log(
        `  ✅ Successfully embedded: ${successCount}/${knowledgeBase.length}`
      );
      ...
      if (successCount === knowledgeBase.length) {
        console.log(
          "🎉 All knowledge chunks successfully embedded and stored!"
        );
      } else if (errorCount > 0) {
        console.log(
          "⚠️  Some chunks failed. You may want to run the script again."
        );
      }
    } catch (error) {
  ...
  async verifyIndex() {
    ...
    console.log(
      "ℹ️  Note: Vector search index must be created manually in MongoDB Atlas"
    );
    console.log("    Index name: knowledge_vector_index");
    console.log(
      "    Field: embedding (vector, 768 dimensions, cosine similarity)"
    );
